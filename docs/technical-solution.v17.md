# novel-multi-agent V17 Technical Solution: Artifact Prune Dry Run

## 方案概览

实现 `planArtifactPrune + CLI artifact-prune`。Core 继续提供纯函数 contract；CLI 负责参数、文件扫描和 JSON 输出；Web 只追加零依赖 inline helper / 可见入口。
