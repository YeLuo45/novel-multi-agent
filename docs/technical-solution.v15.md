# novel-multi-agent V15 Technical Solution: Catalog Compact Output

## 方案概览

实现 `compactArtifactCatalog + verbose option`。Core 继续提供纯函数 contract；CLI 负责参数、文件扫描和 JSON 输出；Web 只追加零依赖 inline helper / 可见入口。
